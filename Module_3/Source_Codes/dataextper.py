...
for j in range(len(y)):
    if(y[j].replace(".apk",".xml") in files):
        c+=1
        print(y[j])
        bit_pattern = [0]*30
        for i in range(len(mal_perm)):
            ...
            try:
                with open (y[j].replace(".apk",".xml"), 'rt') as in_file:        
        ...
        ds.write((str(y[j].encode("utf-8")).replace(",","_").replace(".xml",".apk")+","+(str(bit_pattern).replace("[","").replace("]","")+","+str(z[j])+"\n")).replace("b'","").replace("'",""))
ds.close()
